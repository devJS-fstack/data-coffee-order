export class Vigenere {
    ...
    init() {
        ...
        for (let i = 0; i < this.base.length; i++) {
            const newBase = [...this.base.slice(i, 26), ...this.base.slice(0, i)];
    ...
    encrypt(plainText: string, key: string): string {
        ...
        for (let i = 0; i < ptArr.length; i++) {
            ...
            const indexKey = this.base.findIndex((baseStr) => baseStr === strKey) + 1;
            const indexStr = this.base.findIndex((baseStr) => baseStr === str) + 1;
    ...
    compare({ ct, pt, key }: { ct: string; pt: string; key: string }) {
        const ctProcessed = this.decrypt(ct, key);

        return pt === ctProcessed;
    }
